...
export default function LoginPage() {
  ...
  const whatsappNumber = "+250785140170";
  const whatsappMessage = encodeURIComponent("Hello, I'm interested in this project. I would like to learn more about the POS system features.");
  const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${whatsappMessage}`;

  return (
    <div className="relative min-h-screen flex items-center justify-center p-4">
      <Card className="shadow-none border-0 bg-background/60 backdrop-blur-sm rounded-none w-full max-w-md">
      ...
      <a 
        ...
        className="fixed bottom-8 right-8 z-50 flex items-center gap-3 bg-green-600 hover:bg-green-700 text-white px-6 py-4 rounded-full shadow-2xl transition-all hover:scale-105 active:scale-95 group"
      >
        <MessageCircle className="w-6 h-6 fill-current" />
        <span className="text-sm font-bold uppercase tracking-widest">Chat with Developer</span>
      </a>
    </div>
  );
}